...
def load_model(model_path: str, config, device: str):
    print(f"Loading model from {model_path}")
...
def main():
    # Define default config values before initializing wandb
    default_config = {
        "model_save_dir": "./models",
        "output_dir": "./predictions",
        "data_path": "/home/ubuntu/work/saved_data/Global-Canopy-Height-Map",
        "fixval_csv": "/home/ubuntu/work/saved_data/Global-Canopy-Height-Map/fix_val.csv",
        "batch_size": 8,
        # "device": "cpu",
        "mean": 0,
        "std": 1
    ...
    wandb.init(project='test-000', config=default_config)
    config = wandb.config
    ...
    mean = means.get(dataset_name, [0] * 14)
    std = stds.get(dataset_name, [1] * 14)
    ...
    dataset = FixValDataset(data_path=config.data_path, dataframe=config.fixval_csv)

    models = load_ensemble_models(model_paths, config, device=device)  # Uses correct device
    mean_preds, var_preds, all_preds = predict_with_ensemble(models, dataset, config.batch_size, config.device)
    
    mean_preds = inverse_transform(mean_preds, torch.tensor(mean).view(1, -1, 1, 1), torch.tensor(std).view(1, -1, 1, 1))
    var_preds = inverse_transform(var_preds, torch.tensor(mean).view(1, -1, 1, 1), torch.tensor(std).view(1, -1, 1, 1))
    ...
    """
    torch.save(mean_preds, os.path.join(config.output_dir, "mean_predictions.pt"))
    torch.save(var_preds, os.path.join(config.output_dir, "variance_predictions.pt"))
    torch.save(all_preds, os.path.join(config.output_dir, "all_predictions.pt"))
    """
...
def main():
    ...
    parser.add_argument("--data_path", 
        default="/home/ubuntu/work/saved_data/Global-Canopy-Height-Map", #"/Users/wiebkezink/Documents/Uni Münster/MA/dataset", 
    ...
    parser.add_argument("--fixval_csv", 
        default="/home/ubuntu/work/saved_data/Global-Canopy-Height-Map/fix_val.csv", #"/Users/wiebkezink/Documents/Uni Münster/MA/dataset/fix_val.csv", 
